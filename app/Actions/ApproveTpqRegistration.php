...
{
    public static function execute(TpqRegistration $registration, User $reviewer): TpqProfile
    {
        return DB::transaction(function () use ($registration, $reviewer) {
            // 1. Buat TpqProfile dari data yang ada di registrasi
            ...
            // 2. Assign role head_tpq ke pendaftar, dengan team = tpq_profile_id
            app(PermissionRegistrar::class)->setPermissionsTeamId($tpqProfile->id);
            $registration->applicant->assignRole('head_tpq');

            // 3. Aktifkan user jika belum aktif
            $registration->applicant->update(['is_active' => true]);
